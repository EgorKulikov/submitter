...
pub async fn login(driver: &WebDriver, cookies: Vec<Cookie>) -> WebDriverResult<Vec<Cookie>> {
    driver.goto("https://contest.yandex.com/contest/3/problems/B/").await?;
    ...
    let login: String = Input::with_theme(&dialoguer::theme::ColorfulTheme::default())
        ...
        .unwrap();
    driver.find(By::Id("passp-field-login")).await?.send_keys(login).await?;
    ...
    let confirmation: String = Input::with_theme(&dialoguer::theme::ColorfulTheme::default())
        ...
        .interact_on(&Term::stdout())
        .unwrap();
    driver.find(By::Id("passp-field-confirmation-code")).await?.send_keys(confirmation).await?;
...
pub async fn submit(driver: &WebDriver, url: String, language: String, source: String) -> WebDriverResult<()> {
    ...
    for option in options {
        if option.inner_html().await?.to_lowercase().starts_with(&language.to_lowercase()) {
